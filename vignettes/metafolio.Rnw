...
In an \texttt{R} console (version $\ge 3.0.0$), you can install \texttt{metafolio} by first setting your working directory with \texttt{setwd()} to wherever you saved the package and then running:
...
%report bugs on GitHub: \url{https://github.com/seananderson/metafolio}  (\textit{This website is a
