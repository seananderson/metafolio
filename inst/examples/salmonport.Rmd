...
```{r env-ts, fig.height=5, fig.width=4, fig.cap="Example temperature time series\\label{fig:env-ts}"}
source("plot_env_ts.R")
...
```{r stray-matrix, fig.height=3.7, fig.width=3.9, fig.cap="Example straying matrix. Darker blue colours indicate a greater rate of straying.\\label{fig:stray-matrix}"}
...
```{r thermal-curves, cache=FALSE, fig.height=3.9, fig.width=5.4, fig.cap="Thermal tolerance curves for ten populations. Here, and throughout this document, I have used the same rainbow of colours to identify the salmon populations. The populations are arranged from blue (more productive with cooler temperatures) to red (more productive with warmer temperatures\\label{fig:thermal-curves}"}
...
```{r run-base-example1, fig.height=6.5, fig.width=5, fig.cap="Example simulation with all parameters at default values. I'm only showing a sample of 40 generations (after the burn-in period). From top to bottom: salmon returns each generation (catch plus escapement); Ricker $a$ parameter; the environmental signal (could think of it as temperature or a composite environmental signal like the PDO); number of salmon straying by population; number of straying salmon being received by population; fisheries catch based on escapement MSY calculation; salmon escapement; residuals from the spawner-return curve."}
set.seed(2)
eg1 <- meta_sim(b = rep(1000, 10), n_pop = 10)
plot_sim_ts(eg1, years_to_show = 40)
```

```{r plot-base-corrs, fig.width=4, fig.height=4, fig.cap="A plot comparing the log(returns) between each population. The population colours and numbers match in all figures. Note how populations 1 and 10 have asynchronous returns whereas populations with more similar thermal-tolerance curves (say populations 9 and 10) have more synchronous dynamics. Populations with thermal tolerance curves in the middle (e.g.\ population 6) are less correlated with other populations. Their population dynamics end up primarily driven by demographic stochasticity and less so by temperature-induced systematic changes in productivity."}
plot_correlation_between_returns(eg1, pal = col_pal)
```

```{r plot-base-rickers, fig.width=8, fig.height=5, cache=FALSE, fig.cap="We can also look at samples of the Ricker curves by population (population numbers are in the upper left corners). Here, I've sampled 40 of the $a$ parameters through time for each population. The shaded rectangles to indicate the range of spawners observed in the simulation. The colour of the line indicates the temperature anomaly in that year with blue indicating cooler years and red indicating warmer years. In this example all the populations have the same $b$ parameters. Notice how the productivity is more variable for populations nearer the limits of their thermal tolerance (e.g.\ population 1 vs.\ population 5). Also notice how population 1 is more productive in cool years whereas population 10 is more productive in warm years."}
plot_rickers(eg1, pal = col_pal)
```

```{r plot-various-options-ts-3pops, fig.width=4, fig.height=6.3, fig.cap="Example simulations with different parameter values. I am showing three populations here to make the plots easier to interpret.\\label{fig:sim-param-ts-3}"}
source("example_returns_3.R")
```

```{r plot-various-options-ts, fig.width=4, fig.height=6.3, fig.cap="Example simulations with different parameter values. This version has ten populations.\\label{fig:sim-param-ts-10}"}
source("example_returns_10.R")
```
...
```{r set-a-seed, echo=FALSE}
set.seed(3)
```

```{r monte-carlo-portfolios}
weights_matrix <- create_asset_weights(n_pop = 10, n_sims = 100, 
  weight_lower_limit = 0.01)
mc_ports <- monte_carlo_portfolios(weights_matrix = weights_matrix,
  n_sims = 100, mean_b = 1000)
```

```{r eff-frontier-eg, fig.width=6.5, fig.height=3.5, fig.cap="Efficient frontier of metapopulation portfolios (red dots). Each dot represents a different set of weights of the Ricker $b$ parameters. Metapopulation growth rate is the first difference of the log of total metapopulation abundance. The colours in the right panel correspond to the other figures."}
ef_dat <- plot_efficient_portfolios(port_vals = mc_ports$port_vals, pal = 
  col_pal, weights_matrix = weights_matrix)
```

```{r plot-eff-ports, fig.height=6.5, fig.width=5, fig.cap="Simulation output from the minimum-variance metapopulation."}
min_var_sim <- mc_ports$sims_out[[ef_dat$min_var_port_id]]
plot_sim_ts(min_var_sim, pal = col_pal)
```

```{r fig.width=8, fig.height=5, fig.cap="Example Ricker curves for the minimum-variance metapopulation."}
plot_rickers(min_var_sim, pal = col_pal)
```

```{r plot-portfolio-timeseries, fig.width=6.5, fig.height=3.5, fig.cap="Time series for a random sample of portfolios (grey lines) and the minimum-variance portfolio (white lines). The top panel shows rate of change or metapopulation abundance at each time step and the bottom panel shows the metapopulation abundance."}
source("plot_portfolio_ts.R")
```
