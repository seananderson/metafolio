...
```{r env-ts, fig.height=4, fig.width=4, cache = FALSE, fig.cap="Example temperature time series\\label{fig:env-ts}"}
#library(metafolio)
#or for rapid development:
library(devtools)
load_all("../../") # assuming you're 2 folders deeper than the package
...
```{r thermal-curves, fig.height=3.7, cache = FALSE, fig.width=5.0, fig.cap="Thermal tolerance curves for ten populations. Here, and throughout this document, I have used the same rainbow of colours to identify the salmon populations. The populations are arranged from blue (more productive with cooler temperatures) to red (more productive with warmer temperatures). The vertical lines indicate mean, minimum, and maximum values for one scenario --- you can ignore them for now.\\label{fig:thermal-curves}"}
...
```{r fig.width=6, fig.height=3.75, fig.cap="Example Ricker curves for the minimum-variance metapopulation. (Ignore right now)"}
plot_rickers(min_var_sim, pal = col_pal)
```

```{r plot-portfolio-timeseries, fig.width=6, fig.height=3.5, fig.cap="Time series for a random sample of portfolios (grey lines) and the minimum-variance portfolio (white lines). The top panel shows rate of change or metapopulation abundance at each time step and the bottom panel shows the metapopulation abundance."}
source("plot_portfolio_ts.R")
```

```{r plot-weights-samples, fig.width=6, fig.height=6, fig.cap="A sample of 30 minimum-variance portfolios given the same set of starting parameters as for the example illustrated above. The dashed grey line indicates the weights if all populations had an equal carrying capacity. As it stands, the weights look fairly random to me."}
source("look_at_cluster_dat.R")
```

```{r plot-weights-median, fig.width=4.2, fig.height=3.2, cache = FALSE, fig.cap="Median (solid line) and 25th and 75th quantiles of the weights across 40 minimum-variance portfolios. Just another way of looking at it. (Nothing to see here right now.)"}
source("min_var_ports_dist.R")
```

```{r plot-cons-plans-curves, cache=FALSE, fig.width=3.3, fig.height=5.0, fig.cap="Different ways of prioritizing conservation. The curves show the environmental-tolerance curves for the populations that are conserved. The colours are the same as earlier and identify the different populations. These are in the same order as they are referred to in the next two plots: full range of environmental response diversity, most stable only, one half, other half, most asynchronous.", cache=FALSE}
#Notes #
#-b is "unfished equilibrium stock size"
source("plot_thermal_curves_scenarios.R")
```


```{r plot-cons-plans-mv, fig.width=6.5, fig.height=3.5, fig.cap="Conservation plans in mean-variance space. The dots show portfolios from individual simulation runs. The dark and light shaded areas show 25% and 75% quartiles. The left panel shows a scenario with a stationary environmental signal and strong short-term environmental noise. The right panel shows a scenario with a linearly-increasing environmental signal and minimal short-term environmental noise. These two scenarios bound the two environmental extremes: strong short-term noise with minimal long-term signal vs. a strong long-term signal with minimal short-term signal. Notice what happens to the two *half* conservation strategies. A more realistic scenario combines the two kinds of environmental noise. The efficient frontier is shown for the displayed portfolios. This isn't necessarily the overall efficient frontier.", cache=FALSE}
#Notes #
#-b is "unfished equilibrium stock size"
...
```{r plot-cons-plans-ts, fig.width=6, fig.height=3, fig.cap="Conservation plans in terms of metapopulation abundance. One realization with a periodic (sine wave) environmental signal is shown as an example. The light-grey lines in the background show 100 randomly-sampled portfolios with the same stock-recruit residuals and the same environmental signal. Notice how much less variable the scenarios with balanced response diversity are. The two scenarios with unbalanced response diversity tend to seesaw with the environment."}
source("cons_plans_ts.R")
```

```{r plot-cons-plans-n-mv, fig.width=6.5, fig.height=3.5, fig.cap="Conservation plans in mean-variance space. *This version shows conservation plans with balanced response diversity but different numbers of populations.** The dots show portfolios from individual simulation runs. The dark and light shaded areas show 25% and 75% quartiles. The left panel shows a scenario with a stationary environmental signal and strong short-term environmental noise. The right panel shows a scenario with a linearly-increasing environmental signal and minimal short-term environmental noise. These two scenarios bound the two environmental extremes: strong short-term noise with minimal long-term signal vs. a strong long-term signal with minimal short-term signal. Notice what happens to the two *half* conservation strategies. A more realistic scenario combines the two kinds of environmental noise. The efficient frontier is shown for the displayed portfolios. This isn't necessarily the overall efficient frontier.", cache=FALSE}
source("cons_plans_n.R")
```
