...
plot_sim_ts_simple <- function(x, pal, burn = 1:30, text = "Returns") {
# x is output from a simulation run; it's a list
# pal is the colour palette

to_show <- (max(burn)):(max(burn)+30) # years to show in time series example plots
...
matplot(x$A[to_show, ], type = "l", col = pal, lty = 1, ylab = "Returns", xlab = "Time", xaxt = "n", axes = FALSE, xaxs = "i", ylim = c(580, 1450))
annotate(text)
box(col = "grey50")
my.axis(2)
