...
We developed a 100-year salmon metapopulation simulation model that includes both population dynamics and harvesting along with process, observation, and implementation uncertainty (figure 1). We tested different conservation scenarios under two kinds of environmental regimes. We provide a package `metafolio` for the statistical software `R` [@r2013] as an appendix, to carry out the simulations and analyses described in this paper (Supporting Information).
