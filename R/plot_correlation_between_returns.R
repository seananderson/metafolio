...
#'
#' Create a matrix plot showing the correlation between the returns of
#' each stock/asset.
#'
...
for (i in 1:10) {
  for (j in 1:10) {
    ...
    mtext("log of returns by subpopulation", side = 1, line = 2.5, cex
      = 0.8, outer = TRUE)
    mtext("log of returns by subpopulation", side = 2, line = 2.5, cex
      = 0.8, outer = TRUE, las = 0)
